...
-- name: create-list
INSERT INTO lists (uuid, name, type, optin, status, tags, description, default_messenger) VALUES($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id;
...
WITH l AS (
    UPDATE lists SET
        ...
        default_messenger=(CASE WHEN $8 != '' THEN $8 ELSE default_messenger END),
        updated_at=NOW()
    WHERE id = $1
    RETURNING id, name
),
